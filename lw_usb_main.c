...
uint16_t prev_p1_hp = 100;
uint16_t prev_p2_hp = 100;
...
int main() {
    ...
    XGpio_Initialize(&Gpio_hex, XPAR_GPIO_USB_KEYCODE_DEVICE_ID);
   	...
   	XGpio_Initialize(&HpGpio, XPAR_GPIO_0_DEVICE_ID);
   	XGpio_SetDataDirection(&HpGpio, 1, 0xFFFFFFFF);
	...
	while (1) {
		...
		uint32_t hp_data = XGpio_DiscreteRead(&HpGpio, 1);

		// Unpack the data (P1 is top 16 bits, P2 is bottom 16 bits)
		// NOTE: Adjust the bit shifts if you packed them differently in SystemVerilog!
		uint16_t current_p1_hp = (hp_data >> 16) & 0xFFFF;
		uint16_t current_p2_hp = hp_data & 0xFFFF;

		// Detect if P1 got hit
		if (current_p1_hp < prev_p1_hp) {
			xil_printf("P1 Hit! HP: %d\r\n", current_p1_hp);
		}

		// Detect if P2 got hit
		if (current_p2_hp < prev_p2_hp) {
			xil_printf("P2 Hit! HP: %d\r\n", current_p2_hp);
		}

		// Detect Game Over
		if (current_p1_hp == 0 && prev_p1_hp > 0) {
			xil_printf("PLAYER 2 WINS!\r\n");
		}
		if (current_p2_hp == 0 && prev_p2_hp > 0) {
			xil_printf("PLAYER 1 WINS!\r\n");
		}

		// Update previous HP for the next loop iteration
		prev_p1_hp = current_p1_hp;
		prev_p2_hp = current_p2_hp;
